# Authentication / Authorization (Symfony + Sonata AdminBundle)

> This checklist extends `core/auth.md` and `stacks/symfony/auth.md` for projects using the Sonata bundle. On instruction conflict, this file takes precedence as the most specific layer. The worker loads all three files at once.
...
Forms are generated from `configureFormFields()` in classes `extends AbstractAdmin`. **Recipe-driven recall:** the worker receives a ready list of admin classes and their fields in `recon_bags.addon.sonata.admin_classes.items[*].form_fields` — this is an array of field names from `$form->add('name', ...)`. Walk these items directly and filter by the rules below **before** grepping the source. If `recon_bags.addon.sonata.admin_classes.status == none` — Sonata admin classes are not present in the project.
