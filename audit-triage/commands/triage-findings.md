...
Run `python3 ${CLAUDE_PLUGIN_ROOT}/bin/parse_findings.py <AUDIT_ROOT> --tickets-root <TICKETS_ROOT>` (see the plugin's `bin/` — a stdlib script, no third-party deps). It loads `findings.json` via `json.load` (never regexes the markdown), cross-references `.findings_state.json` resolutions, and prints a normalized index to stdout — counts per verdict plus the `findings_json_sha256` this run is bound to. It also persists `<TICKETS_ROOT>/.work/parsed.json` (the full `Record` list + that hash) so Phases 3 and 5 — separate `python3` invocations, no shared process state — can re-read it instead of re-parsing `findings.json` by hand. Confirm the printed counts against `findings.json`'s own list lengths. Do **not** read the category detail files (`REPORT/<family>.md`) into your own context wholesale at this stage — that happens per-unit in Phase 4, on demand, via Phase 3's bundles.

`needs_validation` entries are **leads, not units of work** — they carry no severity, so they carry no priority; they go to the triage bucket. `hardening` entries are a **separate** bucket, never mixed with leads. Both are still traced: every record from all three `findings.json` lists must land in exactly one unit (Phase 2's coverage check), the triage-bucket units included.

## PHASE 2 — Grouping map + checkpoint

This is judgment work — do it yourself, not the script. Build a classifier `finding → unit` by fix pattern (Principle 2), over the `record_id`s `parse_findings.py` printed/persisted in Phase 1 (`"<verdict>#<index>"`, e.g. `confirmed#3`). Write it to `<TICKETS_ROOT>/.work/grouping.json` as `{"<unit_id>": ["<record_id>", ...], ...}` — one entry per unit, including the triage-bucket units for unmatched `needs_validation`/`hardening` leads. Show the user a **table of proposed units** (priority / id / number of locations / gist) and get a quick "go ahead" — this is cheap and catches a grouping mistake before dozens of files get generated. Do not show source code at this stage.

## PHASE 3 — Bundles + template

Run `python3 ${CLAUDE_PLUGIN_ROOT}/bin/parse_findings.py <AUDIT_ROOT> --tickets-root <TICKETS_ROOT> --grouping <TICKETS_ROOT>/.work/grouping.json`. This re-validates **full coverage** (Principle 3 — zero orphans, zero duplicates, refuses loudly and lists every offender otherwise) and script-exports, per unit, a **bundle** to `<TICKETS_ROOT>/.work/bundles/<unit_id>.md`: the full body text of each constituent `confirmed` finding (fetched from `REPORT/<family>.md`, or `REPORT/manual_review.md` for the manual-review bucket, by `sink_hash`, per Phase 0.1) plus every attached/standalone `needs_validation`/`hardening` note (`findings.json` already carries their full text — `claimed_root_cause`/`blockers`/`text` — no markdown lookup needed for those) and any `[REFUTE_CLAIMED]`/prior-resolution flags. Downstream agents read the bundle, not the raw report again.
