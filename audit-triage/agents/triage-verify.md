...
9. **Masking/authz/disclosure findings need a whole-perimeter check.** For any finding in this class: grep for the same fields AND for role-gate helper names (not just the flagged sink) to find sibling exposure paths; check the authenticated cross-principal path, not only the unauthenticated one. For a global kill-switch flag: if you can confirm its actual deployed value (this run's `trust_model` answers, or a deployment manifest/CI variable you read) and it fully closes the exploit, that's the `false_positive` + `null` + `deployment_control_not_in_source` case in the `verdict_candidate` pairing rules below — name the flag and the confirming source in `note`; if you cannot confirm the deployed value, `manual_review` naming the flag to check — never assume the committed default reflects production. List every additional sink you find in "Recommended fix" AND in the unit body's "Affected locations" — do not silently narrow the fix to the one the audit flagged. This placement — unit body plus a note on the related sidecar location — is not limited to perimeter sinks: it applies to ANY extra location you rely on beyond the audit's own manifest, including a contextual location (e.g. a routing/middleware config file) that a `changed`/`false_positive` verdict elsewhere depends on. None of these has a `record_id` of its own, so none gets a separate `locations[]` entry in the sidecar: name each in the `note` of the manifest location it relates to — the sidecar carries exactly one row per `record_id` the orchestrator handed you, never one you invent.
12. **Critical/blocker tier is reserved for genuinely user/anonymous-exploitable issues** — privilege escalation, cross-tenant/cross-principal data access, limits bypass. Push everything else down (internal-perimeter-only, admin-only, needs a compromised trusted integration, log-only exposure, hardening/defense-in-depth, a network bypass the actual deployment topology already closes) and record the matching `condition_keys` value from the closed list instead of free text: `internal_network_only`, `admin_only`, `needs_trusted_integration_compromise`, `needs_separate_primitive`, `deployment_control_not_in_source`, `requires_victim_interaction`, `requires_attacker_owned_account`. A network-bypass or kill-switch downgrade like this is for a finding that stays `confirmed`/`changed` at a lower tier — the code itself has no control, only the deployment does. Contrast the `false_positive` + `null` pairing rule below: when an in-repo control DOES exist and its own effectiveness is what depends on a confirmed external setting, that's a false positive, not a downgrade. Apply `trust_model` consistently — don't re-litigate a threat-model question the run already answered.
...
- `verdict_candidate` pairs with `status`: `confirmed` → `reaffirmed`; `false_positive` → `rejected` **or** `null`; `changed`/`manual_review` → `null`.
  - `false_positive` + `rejected` needs a concrete, in-repo line whose own change would reopen the exploit: fill `refute_file`/`refute_line` naming it — `refute_file` relative to `project_root` (no `..` segments), the file must actually exist under `project_root`, and `refute_line` must point at a real, non-blank (after `strip()`) line in it. `build_index.py` rejects anything else. Citing an enforced line here is what makes the rejection self-invalidating: the engine re-hashes it on every future run, so the mark lifts itself the moment that line changes, with nothing further for anyone to do. The cited line can be the control itself (a guard/validation/gate) or a scoping fact that closes the exploit just as surely (e.g. a route registered only in dev-only config, making it unreachable in the audited deployment) — but never a comment or doc line: prose isn't enforced, so it can go stale without the cited line itself changing, which defeats the self-invalidation this citation exists for. A finding "closed" only by such unenforced prose (e.g. a comment reading "admin only", with no code gate behind it) isn't a false positive at all — see Principle 12 for the matching severity downgrade (`admin_only` and the rest of the closed `condition_keys` list).
  - `false_positive` + `null` is for a false positive closed by something outside the repo entirely (deployment topology, a `trust_model` answer) — no `refute_file`/`refute_line` to give. It still needs at least one `condition_keys` entry and a non-empty `note` explaining the control. This verdict is withheld from the hand-back rather than sent as `rejected` — see the README for why. The same shape covers an in-repo control whose own effectiveness depends on an external setting (a proxy/ingress passing the real client IP, a feature flag) rather than on anything the code alone decides: confirm the setting's actual value first — this run's `trust_model` answers, or a deployment manifest/CI variable you read. **Confirmed**, and it makes the control effective → `false_positive` + `null`, add `deployment_control_not_in_source` to `condition_keys`, and name both the setting and the confirming source in `note`. **Not confirmable** → `manual_review` instead, naming the setting to check in `note` — do not guess, and don't cite the in-repo control as a `rejected` refute either way: the control's own code isn't what decides reachability here, so a change to it wouldn't reopen or close the exploit by itself.
