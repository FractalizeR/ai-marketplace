...
3. Look for, and report with `file:line` evidence for each:
   - **LOST** — a finding or affected location present in the bundle but missing from the unit's "Affected locations" section and the sidecar's `locations[]`.
   ...
   - **SEVERITY** — a mismatch between the verified facts and the assigned severity tier, including a Critical-tier call that doesn't meet Principle 12's bar (genuinely user/anonymous-exploitable) or a downgrade that skipped the Principle-9 perimeter check for a masking/authz finding.
...
```
<unit_id> / <sink_hash or file:line>: <LOST|DISTORT|OVERCLAIM|SEVERITY> — <one-line description, with your own file:line evidence>
```
