...
3. Look for, and report with `file:line` evidence for each:
   - **LOST** — a finding or affected location present in the bundle but missing from the unit body's "Affected locations" section. (Sidecar `locations[]` coverage against `grouping.json` is machine-checked by `build_index.py`'s sidecar validation — not your job.) The reverse also counts, with one exception: a location described in "Affected locations" should have either a matching `record_id` entry in the sidecar's `locations[]`, or be named in the `note` of a `locations[]` entry it relates to — an extra location the verifier relied on (a bonus sink from a whole-perimeter check, or a contextual location like a routing/middleware config file), which has no `record_id` of its own and so gets no separate sidecar row. Flag it only when it's neither: a claim in the tracker-ready body nothing in the sidecar backs at all.
   ...
   - **OVERCLAIM** — a location marked `confirmed` without your own confirmation in the code, OR a location marked `false_positive` where the code does NOT actually show a control that closes the exploit (the most dangerous direction — a real issue marked away). Exception: a `false_positive` + `null` location carrying `deployment_control_not_in_source` whose `note` names a confirming source (a `trust_model` answer, a deployment manifest/CI variable) — check that named source instead of expecting a code-only control; flag it only if the source itself doesn't actually confirm what the note claims.
