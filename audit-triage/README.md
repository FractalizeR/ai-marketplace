...
A `rejected` mark always cites an in-repo `refute_file`/`refute_line` — the control itself, or a scoping fact that closes the exploit just as surely (e.g. a route registered only in dev-only config), never a comment or doc line. `fr-security-review`'s engine re-hashes that line on every future run, so the rejection lifts itself automatically the moment it changes — the whole reason a line citation is required. A false positive closed by something **outside this repo entirely** (deployment topology, a trust-model answer), or by an in-repo control whose own effectiveness depends on an external setting that was **confirmed** rather than assumed, has no such self-invalidating line to cite: it never becomes a `rejected` verdict, even though the sidecar status is `false_positive` — it stays `withheld`. Sending it as `rejected` anyway would make that false-positive call outlive the fact that justified it, since there's no code location for the engine to notice changing.
