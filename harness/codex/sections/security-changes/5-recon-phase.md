...
**`<SANDBOX_FLAGS>` — choose by `CONSOLE_MODE` (only the recon worker varies):** when `CONSOLE_MODE` is `env` or a custom container/Makefile command the recon process must **execute the project console**, which `-s workspace-write` blocks from reaching the Docker daemon / services (`permission denied` → `console_runtime_failed`, ceiling clamped to medium) — use `--dangerously-bypass-approvals-and-sandbox` (running the console is an explicit operator trust signal; recon is a single process and runs unsandboxed exactly like the orchestrator). For `off` / `auto` use `-s workspace-write --add-dir <REVIEW_ROOT>` (static, or a host `php` runner that needs no daemon; `--add-dir` grants the CONTEXT.md write when the review root is outside `project_root`). **The fan-out wave workers stay `-s workspace-write` + offline regardless** — they never execute the project.

**Author the inputs flag-free (`key=value`).** `codex exec` takes a single positional PROMPT, so the whole read-follow instruction is one quoted argument — never a leading-`--` flag that could collide with `codex exec`'s own options if the quotes are lost. The recon agent maps `diff_files` → `--diff-files=<path>`, `console_mode` (`off` | `auto` | `env` | a container/Makefile command) → `--no-console` / `--console-cmd=<tpl>` / neither (for `env`, the utility reads `FR_SECURITY_CONSOLE_CMD` itself), and `exclude` → `--exclude=<csv>` when it invokes `recon_inventory.py`; the read-follow file is the single source of that mapping.
