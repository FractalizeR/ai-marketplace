...
Skip this whole step (set `CONSOLE_MODE` as noted, proceed to step 4) when **any** of these holds:
...
- **`FR_SECURITY_CONSOLE_CMD` is set in the environment** — check `[ -n "$FR_SECURITY_CONSOLE_CMD" ]`. A launcher (`frsr --console-cmd "docker compose exec -T php bin/console"`) or CI exported the console command out-of-band so its spaces ride the environment intact. Set `CONSOLE_MODE = env` and forward exactly that token to the recon worker — **never** substitute or echo the variable's value into the prompt (its spaces would be re-split). `recon_inventory.py` reads `FR_SECURITY_CONSOLE_CMD` itself. This is the clean, space-safe way to enable container/Makefile console on Codex. (`--no-console` still wins; an explicit `--console-cmd=<tpl>` flag takes precedence when it survived intact.)
