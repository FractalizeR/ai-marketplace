...
1. **Detect whether the stack even has console enrichment.** Run `python3 ${CORE_ROOT}/bin/recon_inventory.py "<PROJECT_ROOT>" --detect` and read `recipe`. Only **Symfony** has console enrichment today (Laravel/generic are fully static). If `recipe != symfony` → nothing to resolve, proceed to step 4 with `CONSOLE_MODE = off`.
