...
```bash
codex exec -m <FAST_TIER> -C <PROJECT_ROOT> -s workspace-write --add-dir <REVIEW_ROOT> --skip-git-repo-check -o "<REVIEW_ROOT>/captures/refute-<i>.capture.txt" "read and follow <CORE_ROOT>/agents/security-refute.md then refute: review_root=<REVIEW_ROOT> batch_index=<i> findings_slice=<≤20 finding rows from the REPORT.md index>"
```
