...
python3 ${CORE_ROOT}/bin/shared/dispatch.py \
  ...
  --allow-gaps \
  --worker-cmd-template 'opencode run -m {model} "<worker SKILL handle> review_root={review_root} project_root={project_root} core_root={core_root} slice_id={slice_id} slice_config={slice_config} mode=changes"'
```
