...
The unifying contract is unchanged: each worker writes `<REVIEW_ROOT>/waves/<slice_id>.md`, and that file is the only thing dedupe reads. On OpenCode the safety net works off the **dispatcher's captured stdout** (under `--capture-dir`), not an in-process response message — and it is **always on**.
...
2. File missing but the worker's captured stdout (under `--capture-dir`) contains the markdown findings body (starting with the `<!-- wave_format: 2 -->` marker as the first non-empty line, as specified in `agents/security.md`) → **you (the orchestrator) recover it**: read that capture and Write the findings to `<REVIEW_ROOT>/waves/<slice_id>.md` yourself, as the Claude harness recovers from a worker's response message. The dispatcher library has a `recover_capture` hook, but the shipped CLI does **not** auto-invoke it — on OpenCode this recovery is the orchestrator's step. That turns the gap into a success.
