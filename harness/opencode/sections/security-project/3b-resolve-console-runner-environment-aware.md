...
- `--no-console` was passed → forward `--no-console` to the recon worker (user's explicit static-only choice).
...
5. Set `CONSOLE_CMD` to the resolved `--console-cmd=...` or `--no-console` (or none) and forward it to the recon worker in step 4.

> The choice is always deterministic on OpenCode: pass `--console-cmd`/`--no-console` to control it. With neither flag on a containerized project, the run proceeds static-only with a recorded `console_gap` (ceiling=medium) — the safe default, surfaced in REPORT.md so nothing is silently dropped.
