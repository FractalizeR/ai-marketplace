...
The unifying contract is unchanged: each worker writes `<REVIEW_ROOT>/waves/<slice_id>.md`, and that file is the only thing dedupe reads. On OpenCode the safety net works off the **dispatcher's captured stdout** (under `--capture-dir`), not an in-process response message.
...
2. If the file is missing but the worker's captured stdout (under `--capture-dir`) contains the markdown findings body (starting with the `<!-- wave_format: 2 -->` marker as the first non-empty line, as specified in `agents/security.md`) → **you (the orchestrator) recover it**: read that capture and Write the findings to `<REVIEW_ROOT>/waves/<slice_id>.md` yourself — exactly as the Claude harness recovers from a worker's response message (the worker→file contract is the only thing dedupe reads). The dispatcher library exposes a `recover_capture` hook for this, but the shipped CLI does **not** auto-invoke it, so on OpenCode the recovery is this orchestrator step. That turns the gap into a success.
