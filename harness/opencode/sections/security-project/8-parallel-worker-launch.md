...
OpenCode has no in-process subagent fan-out — each wave runs as its own external `opencode run` process (targeting the worker via `--agent security`). The shared dispatcher (`shared/dispatch.py`, `dispatch_waves`) launches one worker per plan slice, **≤6 concurrent** (`ThreadPoolExecutor`, hard-capped), and enforces the wave-file contract: each worker writes its findings to `<REVIEW_ROOT>/waves/<slice_id>.md`.
...
python3 ${FR_SECURITY_CORE_ROOT}/bin/shared/dispatch.py \
  ...
  --allow-gaps \
  --worker-cmd-template 'opencode run -m {model} --agent security "review_root={review_root} project_root={project_root} core_root={core_root} slice_id={slice_id} slice_config={slice_config} mode=project"'
```
